...
**Sintomas:** Errores 500 en API, logs muestran fallo de conexion a PostgreSQL, datos inconsistentes.

**Pasos de recuperacion:**

1. Verificar estado de PostgreSQL en https://status.postgres.com — si es incidente de plataforma, esperar y monitorear.
2. Si es problema de configuracion: verificar `DATABASE_URL` en VPS Variables.
3. Acceder a PostgreSQL Dashboard → Database → Backups.
4. Identificar el ultimo backup valido (PostgreSQL hace backups diarios automaticos en plan Pro).
5. Iniciar restauracion desde PostgreSQL Dashboard → Point-in-time recovery si esta disponible.
...
**Escalada:** Si PostgreSQL no responde en 30 minutos, contactar soporte premium de PostgreSQL con ticket urgente.

---

## Escenario 3: Push falla (notificaciones push no llegan)

**RTO objetivo:** N/A — hay fallback automatico

**Sintomas:** Alertas emitidas pero ciudadanos no reciben push notifications, logs muestran error Push.

**Pasos de recuperacion:**

1. Verificar estado de Notificaciones en https://status.notificaciones.google.com.
2. Si es incidente de Notificaciones: el sistema debe continuar operando — las alertas se guardan en BD.
...
5. Una vez Notificaciones recuperado: revisar y limpiar cola de mensajes duplicados si los hubo.
...
1. **Evaluacion inicial (0-30 min):**
   - Verificar si es incidente de seguridad: revisar logs de VPS y PostgreSQL audit log.
...
2. **Contencion (30-60 min):**
   ...
   - Preservar logs actuales exportandolos desde VPS y PostgreSQL.

3. **Recuperacion de infraestructura (1-3 h):**
   - Provisionar nuevo ambiente en VPS desde el ultimo Dockerfile en git.
   - Restaurar BD desde el ultimo backup valido en PostgreSQL.
...
| Soporte VPS | - | https://
| Soporte PostgreSQL | - | https://postgres.com/support |
| Soporte Notificaciones | - | https://notificaciones.google.com/support |
