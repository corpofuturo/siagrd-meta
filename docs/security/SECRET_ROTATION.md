...
| `VPS_SSH_KEY (GitHub Secret)| 90 dias | Registrar en bitacora |

---

## SUPABASE_SERVICE_ROLE_KEY (cada 90 dias)

1. Ingresar a PostgreSQL Dashboard → Proyecto SIAGRD → Settings → API.
2. Generar nueva `service_role` key desde la opcion "Roll API keys".
3. Actualizar el secret en VPS: Panel → Variables → `SUPABASE_SERVICE_ROLE_KEY`.
...
6. Hacer deploy de backend en VPS para aplicar el nuevo valor.
7. Verificar que el endpoint `/health` responda `200`.
8. Revocar la key anterior en PostgreSQL una vez confirmada la nueva.
...
1. Ingresar a Notificaciones Console → Proyecto SIAGRD → Configuracion del proyecto → Cuentas de servicio.
...
4. Actualizar en VPS: `FIREBASE_PRIVATE_KEY` y `FIREBASE_CLIENT_EMAIL`.
5. Actualizar en GitHub Secrets si se usan en CI.
6. Hacer deploy de backend.
7. Verificar envio de notificacion Push de prueba.
8. Eliminar la cuenta de servicio anterior en Notificaciones Console.
...
1. Ingresar a VPS Dashboard → Account → Tokens.
2. Crear nuevo token con nombre descriptivo (ej: `siagrd-ci-2025-09`).
3. Actualizar en GitHub Secrets: Settings → Secrets → Actions → `VPS_SSH_KEY (GitHub Secret)
4. Ejecutar un pipeline de CI en una rama de prueba para verificar que el deploy funciona.
5. Revocar el token anterior en VPS Dashboard.
...
3. Revisar logs de VPS, PostgreSQL y Notificaciones para detectar accesos no autorizados en las ultimas 72 horas.
