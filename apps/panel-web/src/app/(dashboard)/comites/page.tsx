...
const TIPO_COLORS: Record<TipoComite, { bg: string; text: string; border: string; avatar: string }> = {
  CONGRD: { bg: 'bg-[#7C3AED]/15', text: 'text-[#6d28d9]', border: 'border-[#7C3AED]/40', avatar: 'bg-[#7C3AED]' },
  CDGRD:  { bg: 'bg-[#2563EB]/15', text: 'text-[#1d4ed8]', border: 'border-[#2563EB]/40', avatar: 'bg-[#2563EB]' },
  SDGRD:  { bg: 'bg-[#4F46E5]/15', text: 'text-[#4338ca]', border: 'border-[#4F46E5]/40', avatar: 'bg-[#4F46E5]' },
  CMGRD:  { bg: 'bg-[#0D9488]/15', text: 'text-[#0f766e]', border: 'border-[#0D9488]/40', avatar: 'bg-[#0D9488]' },
};
  ...
  return (
    <form onSubmit={handle} className="flex flex-col gap-4">
      ...
      <div className="flex items-center gap-2.5 py-1">
        ...
        />
        <label htmlFor="activo-check" className="text-[#6b7280] text-sm">Comité activo</label>
      </div>
    ...
    <div className="flex-1">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {STAT_TIPOS.map(({ tipo, icon, desc }) => {
          ...
          return (
            <div key={tipo} className={`bg-[#ffffff] border border-[#e5e7eb] rounded-xl p-4 relative overflow-hidden`}>
              ...
              <p className="text-[#111827] text-2xl font-bold font-mono">{counts[tipo]}</p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#f3f4f6] bg-[#f9fafb]">
                <th className="text-left px-5 py-3 text-xs text-[#9ca3af] font-semibold uppercase tracking-wider">Comité</th>
            ...
            <tbody>
              {filtrados.map(c => {
                ...
                return (
                  <tr key={c.id} className="border-b border-[#f3f4f6] last:border-0 hover:bg-[#f9fafb] transition-colors group">
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-[#111827] font-semibold text-sm leading-tight">{c.nombre}</p>
                          <div className="flex items-center gap-2 mt-0.5">
                            <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${colors.bg} ${colors.text} border ${colors.border}`}>
                              {TIPO_SHORT[c.tipo]}
                            </span>
                            {c.correo && <span className="text-[#9ca3af] text-[10px] font-mono">{c.correo}</span>}
                          </div>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3.5 text-[#6b7280] text-xs">{c.municipio_nombre ?? <span className="text-[#9ca3af]">—</span>}</td>
                    <td className="px-4 py-3.5 text-[#6b7280] text-xs">{c.presidente ?? <span className="text-[#9ca3af]">—</span>}</td>
