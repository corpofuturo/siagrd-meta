      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            ...
            className="px-3 py-1.5 bg-[#f3f4f6] border border-[#e5e7eb] rounded text-sm text-[#111827] hover:bg-[#e5e7eb] transition-colors"
          >
            Actualizar
          </button>
        </div>
        ...
        {loading && (
          <div className="text-center py-12 text-[#6b7280]">Cargando reportes...</div>
        )}

        {!loading && reportes.length === 0 && (
          <div className="text-center py-12 bg-[#ffffff] border border-[#e5e7eb] rounded-lg">
            <p className="text-[#6b7280]">Sin reportes pendientes</p>
          </div>
        )}
        ...
        <div className="space-y-3">
          {reportes.map((reporte) => (
            <div
              key={reporte.id}
              className="bg-[#ffffff] border border-[#e5e7eb] rounded-lg p-4 flex gap-4"
            >
              {/* Foto miniatura */}
              {reporte.foto_url && (
                <div className="flex-shrink-0">
                  ...
                  <img
                    ...
                    alt="Foto reporte"
                    className="w-20 h-20 object-cover rounded border border-[#e5e7eb]"
                  />
                </div>
              )}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2 mb-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {reporte.municipio && (
                      <span className="text-xs text-[#6b7280]">{reporte.municipio}</span>
                    )}
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 bg-[#f3f4f6] hover:bg-[#e5e7eb] border border-[#e5e7eb] text-[#6b7280] rounded text-xs transition-colors disabled:opacity-40"
                  >
                    Rechazar
                  </button>
                  ...
                  {(reporte.lat && reporte.lng) && (
                    <a
                      ...
                      className="px-3 py-1.5 bg-[#f3f4f6] hover:bg-[#e5e7eb] border border-[#e5e7eb] text-[#6b7280] rounded text-xs transition-colors"
                    >
                      Ver en mapa
                    </a>
                  )}
